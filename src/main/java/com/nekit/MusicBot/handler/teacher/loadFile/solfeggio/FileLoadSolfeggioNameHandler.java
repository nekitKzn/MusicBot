...
public class FileLoadSolfeggioNameHandler implements Handler {
    ...
    public Object handle(Message message) {
        var keyboard = getKeyboardDefault(StateBot.FILE_LOAD_SOLFEGGIO_SUCCESSFULLY);
        if (message.hasPhoto()) { // если фото, то ввести название нужно обязательно
            keyboard = null;
        }
        fileService.saveNewFile(message, FileLesson.SOLFEGGIO_FILE);
        userService.updateUserState(message.getChatId(), StateBot.FILE_LOAD_SOLFEGGIO_SUCCESSFULLY);
